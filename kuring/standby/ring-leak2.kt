...
fun add_socket_eventfd_read(ring:CPointer<io_uring>, fd:Int):Unit{
	val __FUNCTION__="add_socket_eventfd_read"
    ...
    memcpy(sqe.pointed.user_data.ptr, conn_i.ptr, sizeof(conn_i));
}

fun add_socket_pollin(ring:CPointer<io_uring>, fd:Int):Unit{
	val __FUNCTION__="add_socket_pollin"
...
fun server_thread(void:CPointer<ByteVar>arg:CPointer<void>{
	val __FUNCTION__="server_thread"
    ...
    // bind and listen
    if (bind(sock_listen_fd, (struct sockaddr *) serv_addr.ptr, sizeof(serv_addr)) < 0) {
    ...
    while (1) {
        ...
        io_uring_for_each_cqe(ring.ptr, head, cqe) {
            ...
            count++;
            memcpy(conn_i.ptr, cqe.pointed.user_data.ptr, sizeof(conn_i));
...
fun client_thread(void:CPointer<ByteVar>arg:CPointer<void>{
	val __FUNCTION__="client_thread"
