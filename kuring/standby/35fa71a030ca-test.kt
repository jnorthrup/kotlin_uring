...
fun thread_start(void *(*fn)(void *), void:CPointer<ByteVar>arg:Unit{
	val __FUNCTION__="thread_start"
...
fun event_init(ev:CPointer<event_t>):Unit{
	val __FUNCTION__="event_init"

    ev.pointed.state = 0;
}

fun event_reset(ev:CPointer<event_t>):Unit{
	val __FUNCTION__="event_reset"

    ev.pointed.state = 0;
}

fun event_set(ev:CPointer<event_t>):Unit{
	val __FUNCTION__="event_set"

    if (ev.pointed.state)
        exit(1);
    __atomic_store_n(ev.pointed.state.ptr, 1, __ATOMIC_RELEASE);
    syscall(SYS_futex, ev.pointed.state.ptr,  FUTEX_WAKE or FUTEX_PRIVATE_FLAG );
}

fun event_wait(ev:CPointer<event_t>):Unit{
	val __FUNCTION__="event_wait"

    while (!__atomic_load_n(ev.pointed.state.ptr, __ATOMIC_ACQUIRE))
        syscall(SYS_futex, ev.pointed.state.ptr,  FUTEX_WAIT or FUTEX_PRIVATE_FLAG , 0, 0);
}

fun event_isset(ev:CPointer<event_t>):Int{
	val __FUNCTION__="event_isset"

    return __atomic_load_n(ev.pointed.state.ptr, __ATOMIC_ACQUIRE);
}

fun event_timedwait(ev:CPointer<event_t>, timeout:uint64_t):Int{
	val __FUNCTION__="event_timedwait"
    ...
    for (;;) {
        ...
        syscall(SYS_futex, ev.pointed.state.ptr,  FUTEX_WAIT or FUTEX_PRIVATE_FLAG , 0, ts.ptr);
        if (__atomic_load_n(ev.pointed.state.ptr, __ATOMIC_RELAXED))
            return 1;
        now = current_time_ms();
...
static struct threads:thread_t[16];

static void execute_call(call:Int);

static running:Int;

fun thr(void:CPointer<ByteVar>arg:CPointer<void>{
	val __FUNCTION__="thr"

    th:CPointer<thread_t> = (struct thread_t *) arg;
    for (;;) {
        event_wait(th.pointed.ready.ptr);
        event_reset(th.pointed.ready.ptr);
        execute_call(th.pointed.call);
        __atomic_fetch_sub(running.ptr, 1, __ATOMIC_RELAXED);
        event_set(th.pointed.done.ptr);
    }
    return 0;
}
    ...
    for (call in 0 until  3) {
        for (thread = 0; thread < (int) (sizeof(threads) / sizeof(threads[0]));
            ...
            if (!th.pointed.created) {
                ...
                event_init(th.pointed.ready.ptr);
                event_init(th.pointed.done.ptr);
                event_set(th.pointed.done.ptr);
                thread_start(thr, th);
            }
            if (!event_isset(th.pointed.done.ptr))
                continue;
            event_reset(th.pointed.done.ptr);
            th.pointed.call = call;
            __atomic_fetch_add(running.ptr, 1, __ATOMIC_RELAXED);
            event_set(th.pointed.ready.ptr);
            event_timedwait(th.pointed.done.ptr, 45);
            break;
        }
    }
