...
int
expect_fail(fd:Int, opcode:UInt, void:CPointer<ByteVar>arg
...
#define MAXFDS (sizeof:CPointer<UINT_MAX>(int))

void *
map_filebacked(size:size_t) {
    fd:Int, ret;
    void:CPointer<ByteVar>addr
...
test_max_fds(uring_fd:Int) {
    status:Int = 1;
    ret:Int;
    void:CPointer<ByteVar>fd_as /* file descriptor address space */
    fdtable_fd:Int; /* fd for the file that will be mapped over and over */
    io_fd:Int; /* the valid fd for I/O -- /dev/null */
    fds:CPointer<Int>; /* used to map the file into the address space */
    char template[32] = "io_uring_register-test-XXXXXXXX";
    i:ULong , nr_maps, nr_fds;
     ...
     */
    fd_as = mmap(NULL, sizeof:CPointer<UINT_MAX>(int),  PROT_READ or PROT_WRITE ,
    ...
    nr_maps = (sizeof:CPointer<UINT_MAX>(int)) / (128 * 1024 * 1024);
    ...
    close(io_fd);
    close(fdtable_fd);
    ret = munmap(fd_as, sizeof:CPointer<UINT_MAX>(int));
    if (ret != 0) {
...
test_iovec_nr(fd:Int) {
    ...
    iovs = malloc(sizeof:CPointer<nr>(struct iovec));
    if (!iovs) {
...
test_iovec_size(fd:Int) {
    ...
    buf = mmap(NULL, pagesize:CPointer<2>,  PROT_READ or PROT_WRITE ,
                MAP_PRIVATE or MAP_ANONYMOUS , -1, 0);
    assert(buf != MAP_FAILED);
    ret = munmap(buf + pagesize, pagesize);
    assert(ret == 0);
    iov.iov_base = buf;
    iov.iov_len = pagesize:CPointer<2>;
    status |= expect_fail(fd, IORING_REGISTER_BUFFERS, iov.ptr, 1, EFAULT);
    munmap(buf, pagesize);
    ...
    if (buf == MAP_FAILED) {
        ...
        /*
         * This should succeed, so as:Long RLIMIT_MEMLOCK is
         * not exceeded
         */
        iov.iov_base = buf;
...
int
ioring_poll(ring:CPointer<io_uring>, fd:Int, fixed:Int) {
    ret:Int;
...
fun test_shmem(void):Int{
	val __FUNCTION__="test_shmem"
...
int
main(argc:Int, argv:CPointerVarOf<CPointer<ByteVar>>) {
    fd:Int, ret;
    status:UInt = 0;
    p:io_uring_params;
    rlim:rlimit;
