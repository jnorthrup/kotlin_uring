...
class acceptLink {
    var pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
    var pthread_cond_t cond = PTHREAD_COND_INITIALIZER;

    var recv_thread_ready:Int = 0;
    var recv_thread_done:Int = 0;

    fun  signal_var1(var1:CPointer<Int> ) {
    ...
    static send_thread:CPointer<Bytevar1> (arg:CPointer<Bytevar1> )
    {
        data:CPointer<data> = arg;
        ret:Int;

        wait_for_var1(recv_thread_ready.ptr);
        ...
        wait_for_var1(recv_thread_done.ptr);

        close(s0);
        return NULL;
    }

    fun recv_thread(arg: CPointer<Bytevar1>): CPointer<Bytevar1> {
        data:CPointer<data> = arg;
        ring:io_uring;
        i:Int, ret;
        ...
        if (i >= 100) {
            ...
            data.pointed.stop = 1;
            signal_var1(recv_thread_ready.ptr);
            goto out;
        }
        ...
        signal_var1(recv_thread_done.ptr);
    ...
    int main(argc:Int, argv:CPointer<Bytevar1>[])
    {
        if (argc > 1)
            return 0;
