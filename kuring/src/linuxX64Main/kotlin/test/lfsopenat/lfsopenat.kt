...
//    HasPosixErr.posixRequires(! (argc > 0)){"no args accepted"}
    ...
    HasPosixErr.posixRequires(!(dfd < 0)) { "open /tmp: " + posix_strerror1(platform.posix.errno) }
...
fun prepare_file(dfd: Int, fn: String): Int {
    ...
    HasPosixErr.posixRequires(!(res < 0)) { "prepare/pwrite: " + posix_strerror1(platform.posix.errno) }
...
fun test_drained_files(dfd: Int, fn: String, linked: Boolean, prepend: Boolean): Int = nativeHeap.run {
    ...
    val iov: iovec = alloc {
        iov_base = buffer.pin().objcPtr().toLong().toCPointer()
        iov_len = 128.toULong()
    }
    var ret: Int
    val fd: Int
    ...
    ret = io_uring_submit(ring.ptr)
    HasPosixErr.posixRequires(!(ret != 1 + to_cancel)) { "failed to submit openat: " + posix_strerror1(-ret) }

    fd = posixDup(ring.ring_fd)
    HasPosixErr.posixRequires(!(fd < 0)) { "dup() failed: " + posix_strerror1(-fd) }
